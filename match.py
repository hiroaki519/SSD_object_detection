# -*- coding: utf-8 -*-
"""
Created on Fri Apr 28 13:56:39 2023
...
def intersect(box_a, box_b):
    ...
    #以下のtorch.minとtorch.maxで、box_aとbox_bの
    #すべての組み合わせに対して、双方のボックスが重なる部分の
    #４辺の座標[xmin, ymin, xmax, ymax]を求める
    
    #box_aとbox_bの[xmax, ymax]のすべての組み合わせに対して
    #min()で小さいほうのxmax, ymaxを抽出(x軸、y軸の最大値を切り詰め)
    
    #A(0)からA(n)までB(1)~B(n)を繰り返し組み合わせることで
    #すべての組み合わせを作る
    #A(0)に対してB(0)~B(n)を組み合わせる
    #　　　↓
    ...
    max_xy = torch.min(
        ...
        #(Aの数、２)をunsqueeze(1)で(Aの数、１，２[x,y])に拡張して
    ...
    # box_aとbox_bの[xmin,ymin]のすべての組み合わせに対して
...
def jaccard(box_a, box_b):
    ...
    #Return(Tensor):
        #BBoxとすべてのDBoxの組み合わせにおけるジャッカード係数(IoU)
    ...
    # box_a(0)の面積を求めてbox_bの数だけ並べて、これをbox_a（n)まで繰り返す
    
    #1.(Aの数[面積],)をunsqueeze(1)で1の次元を拡張する(Aの数[面積], 1)
    #2.expand_as(inter)でAの面積をBの数だけ並べて(Aの数、Bの数)のかたちにする
    ...
    #2.expand_as(inter)でBの面積の列をAの数だけ並べて(Aの数、Bの数)のかたちにする
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    #[[<BBox(0)に対する>　DBox（０）のIoU値, ..., DBox(n)のIoU値]、
    #   ...
    ...
    #best_prior_idx: (BBoxの数、　１[BBoxにマッチしたDBoxのインデックス])
    ...
    #overlaps(正解BBoxの数、DBoxの数)に対し、
    #overlaps.max(0, keepdim=True)で0次元[BBox(0), ..., BBox(n)]
    #から最高IoU値と、そのIoU値を出したBBoxのインデックスを取得
    ...
    # (BBoxの数、1[BBoxにマッチしたDBoxのインデックス])の１の次元を削除
    ...
    # DBoxのオフセット情報loc(DBoxの数、４)を格納
    loc_t[idx] = loc
    # conf_t[現在のミニバッチのインデックス]に
    # 正解ラベルconf(DBoxの数[0への置き換え処理後の正解ラベル],)格納
    conf_t[idx] = conf
...
def encode(matched, priors, variances):
    ...
    # g_wh=[(log(BBoxのw/DBoxのw) / 0.2), (log(BBoxのh/DBoxのｈ)/0.2)]
    g_wh = torch.log(g_wh) / variances[1]
    
    # Δcx,　Δcy, Δw, Δhを1の次元で連結して
